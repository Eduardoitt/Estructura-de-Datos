...
namespace Tarea4
{
    class Program
    {
        ...
        static void Mayor(Queue<string> nombres, Queue<int> edades,int F,int n)
        {
            ...
            NomyEdad(copiedad,copinom,n,nombres,edades,F,mayor,mon);
            
        }
        //impresion de nombres y edades
        static void NomyEdad(int[] copiedad, string[] copinom, int n, Queue<string> nombres, Queue<int> edades,int F,int mayor, string nom)
        {
            for (int i = 0; i <F; i++)
            {
                Console.WriteLine("Nombre "+copinom[i]+" edad "+ copiedad[i]);
            }
            Eliminar(nombres, edades,F,mayor,nom);
        }
        //eliminacion
        static void Eliminar(Queue<string> nombres, Queue<int> edades,int F,int mayor,string nom)
        {
            int I=0;
            string otro,ot;
            do
        ...
        static void ImpNyE(int mayor, string nom)
        {
